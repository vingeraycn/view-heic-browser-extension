...
| 1.3: 2026-08-02 to 08-13 | 63 | 564,850 | 564,815 | Reported users diverge sharply from the roughly 3,000 store users, so the result cannot be interpreted as real user loss. |
...
| --- | --- | --- |
| `extension_active` | Attached once to the first successfully reported real event on each local calendar day | `activity_source`, `engagement_time_msec=1` |
...
| `conversion_completed` | A real conversion workflow ends with success, partial success, or failure | `surface`, `trigger`, `outcome`, `attempted_count`, `success_count`, `failure_count`, `duration_ms`, optional `error_type` |
